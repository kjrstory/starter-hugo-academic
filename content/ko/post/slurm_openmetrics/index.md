...
이 글에서는 실제 멀티 GPU 클러스터에서 이 기능을 구성한 과정을 공유합니다. 2026년 3월 현재, Grafana Labs 대시보드 마켓플레이스에 25.11 네이티브 메트릭 기반 대시보드는 등록되어 있지 않아, 직접 만든 대시보드도 함께 공유합니다.
...
```
slurmctld ← squeue/sinfo → prometheus-slurm-exporter(:9341) ← Prometheus ← Grafana
```

별도의 exporter 바이너리가 주기적으로 `squeue`, `sinfo`, `sdiag` 등의 CLI 명령을 실행하고, 그 출력을 파싱해서 Prometheus 메트릭으로 변환하는 구조입니다. 잘 동작하지만 몇 가지 불편함이 있었습니다:
...
공식 네이티브 메트릭이 도입됨에 따라, 커뮤니티 exporter들의 필요성은 자연스럽게 줄어들 것으로 보입니다. ([Slurm 25.11 릴리즈 노트](https://slurm.schedmd.com/news.html), [메트릭 가이드](https://slurm.schedmd.com/metrics.html))
...
| 기존 Exporter | 25.11 Native | 설명 |
|---|---|---|
| `slurm_cpus_alloc` | `slurm_jobs_cpus_alloc` | 총 할당 CPU |
| `slurm_cpus_idle` | `sum(slurm_node_cpus_idle)` | 총 유휴 CPU |
| `slurm_cpus_total` | `sum(slurm_node_cpus)` | 총 CPU |
| `slurm_nodes_alloc` | `slurm_nodes_alloc` | 할당 노드 (이름 동일) |
| `slurm_nodes_idle` | `slurm_nodes_idle` | 유휴 노드 (이름 동일) |
| `slurm_node_cpu_alloc{node="..."}` | `slurm_node_cpus_alloc{node="..."}` | 노드별 CPU |
| `slurm_node_mem_alloc{node="..."}` | `slurm_node_memory_alloc_bytes{node="..."}` | 노드별 메모리 |
| `slurm_scheduler_mean_cycle` | `slurm_sched_mean_cycle` | 스케줄러 사이클 |
| `slurm_scheduler_backfilled_jobs_since_start_total` | `slurm_backfilled_jobs` | 백필 작업 수 |
| `slurm_account_fairshare{...}` | — | (네이티브에 없음) |

일부 메트릭은 이름만 다르고 일부는 체계가 완전히 다릅니다. 특히 파티션별, 사용자별 메트릭은 네이티브 쪽이 훨씬 풍부합니다. 반면 fairshare 메트릭은 아직 네이티브에 포함되어 있지 않습니다.

## Grafana 대시보드

2026년 3월 현재, Grafana Labs 대시보드 마켓플레이스에 등록된 Slurm 대시보드는 모두 기존 exporter 기반입니다:
...
25.11 네이티브 메트릭 기반 대시보드는 아직 없습니다. 아래는 직접 구성한 대시보드입니다.
...
25.11로 업그레이드했다고 기존 exporter를 바로 제거할 필요는 없습니다. 병행 운영하면서 점진적으로 전환할 수 있습니다:

1. 네이티브 메트릭 scrape config를 추가합니다.
2. Grafana에 새 대시보드를 만들어 네이티브 메트릭을 확인합니다.
3. 기존 대시보드와 비교하며 데이터 정합성을 확인합니다.
4. 확인이 되면 기존 exporter를 제거합니다.

저희 클러스터에서도 기존 `prometheus-slurm-exporter(:9341)`와 네이티브 메트릭을 약 2주간 병행 운영한 뒤 전환했습니다.
...
| Grafana 대시보드 | 마켓플레이스에 다수 | 아직 없음 (직접 구성 필요) |
