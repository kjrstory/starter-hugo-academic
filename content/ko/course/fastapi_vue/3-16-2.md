...
구상한 API를 토대로 스키마 파일을 다음과 같이 작성한다. 파일의 위치는 domain/comment/comment_schema.py이다. Answer의 스키마 파일과 유사하다.
...
파일의 위치는 domain/comment/comment_crud.py이다. 역시 Answer의 crud파일과 거의 유사하다. 
