...
GROMACS라는 오픈소스를 예로 들면, Intel Cascade Lake에서 다른 컴파일 옵션을 사용했을 때 성능 차이가 70%에 이르렀습니다. 저는 OpenFOAM에서도 유사한 테스트를 진행했지만, 예상과 달리 큰 차이가 나타나지 않았습니다. 이는 소프트웨어의 코드 구조 차이에서 기인한 것으로 보이며, 이러한 성능 차이의 원인을 분석하는 것도 매우 중요한 연구 주제입니다.
...
배포판마다 별도의 패키지로 구분하며 각각 레시피 파일이 존재합니다.
Spack에서는 2017년경에 OpenCFD 배포판을 기준으로 다른 배포판의 레시피가 작성되었습니다.
...
OpenFOAM 레시피에서도 매 버전마다 url을 작성했습니다.
...
처음에는 바뀐 부분이 매우 단순했기 때문에 매우 쉽게 통과할 것으로 기대했지만 리뷰에서 부정적인 의견을 받았습니다. 이 패키지의 가장 첫번째 버전인 2.3.1은 github의 url을 사용하지 않고 소스포지의 url을 사용하게 되는데 이것이 함수에 담겨있지 않았기 때문입니다. 그래서 2.3.1에서는 소스포지의 url, 5.0이하에서는 x 규칙이 들어가게 규칙을 추가하였습니다.
...
decompostion 메쏘드는 격자를 병렬 해석하기 위해 여러 파트로 나누는 작업입니다.
...
Foundation 배포판에는 Zoltarn 패키지가 사전 패키지가 없었기 때문에 추가해야 합니다.
한가지 유의할 것은 단순히 패키지만 필요로 하는것이 아니라 패치가 필요하다는 것입니다.
일단 zoltan패키지가 shared버전(`zoltan+shared`)으로 필요합니다.
...
Additive Foam이라고 하여 Additive Manufacturing 솔버를 개발한 기관에서는 spack에 새로운 패키지로 등록하기 원했습니다.
단 이들이 처음 제시한 commit은 DSL의 문법을 충분히 사용하지 않은 상황이었습니다.
...
OpenRadioss는 전처리와 솔버가 나누어져있고 빌드도 두차례를 하게 됩니다.
