...
If I understand correctly, you have decided to use a new build directory instead of the existing FDS approach where the executable files are placed in the build folder such as "Build/ompi_gnu_linux". Instead, you would like to install the executable files in the bin directory of the FDS installation folder. You have created an install function that creates a bin folder and places the necessary files in it. Is that correct?

If so, please provide the code or specify the details of the install function so that I can assist you further.
